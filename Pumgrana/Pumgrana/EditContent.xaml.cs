...
namespace Pumgrana
{
    public partial class EditContent : PhoneApplicationPage
    {
        ...
        private void ApplyAddTag(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (DT.list_Tag.Count((tmp) => tmp.Subject == data) > 0)
                {
                    is_Open = false;
                    TagToApply Tag = DT.list_Tag.First((tmp) => tmp.Subject == data);
        ...
        void HttpClient_InsertTags(object sender, object output)
        {
            ...
            if (data.status == 201)
            {
                ...
                MessageBox.Show("Tag " + DT.list_Tag.Last().Subject + " has been added.");
            }
            else
            {
